...
class TokenBaseModel(Base):
    ...
    token: Mapped[Optional[str]] = mapped_column(String(64), nullable=False, unique=True, default=generate_secure_token)
    expires_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(timezone.utc) + timedelta(hours=24))
...
class PasswordResetTokenModel(TokenBaseModel):
    ...
    def __repr__(self):
        return f"<PasswordResetTokenModel {self.user_id}: {self.token} {self.expires_at}>"
...
class RefreshTokenModel(TokenBaseModel):
    ...
    token: Mapped[str] = mapped_column(String(512), nullable=False, unique=True, default=generate_secure_token)
